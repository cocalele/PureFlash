 ...
 */
typedef struct pf_message_head
{
	int32_t		magic_num;		///< 0x3553424e, magic number.
	int32_t		msg_type;		///< type.
	int32_t		transaction_id;	///< transaction id. bit[0~8]:ictx->node_cache index, bit[9~21]:submitted io counter, bit[22~31]:submitted io index
	int64_t		slba;			///< start of LBA.
	int32_t		data_len;		///< the length of  s5message's data, unit is byte.
	uint64_t	volume_id;		///< volume id, s5message assosiated.
	int32_t		user_id;		///< user id, not to be used currently.
	int32_t		pool_id;		///< pool id, not to be used currently.
	int32_t		nlba;			///< count of LBAs.
	int32_t		obj_ver;		///< s5 block object version, not to be used currently.
	int32_t		listen_port;	///< listen port for receiving snap_change_notify in open-image listen_port for toe port in open-image-reply.
	int32_t		snap_seq;		///< snap sequence, not to be used currently.
	int32_t		status;			///< handle status of this s5message, valid in reply message..
	...
	int8_t		reserve[1];		///< reserve.
} pf_message_head_t;

/**
 *   s5message's tail data structure definition.
 */
typedef struct pf_message_tail
{
	...
	*/
	int32_t		flag;
	int32_t		crc;	///< crc value of s5message's data.
} pf_message_tail_t;
...
{
    pf_message_head_t head; ///< s5message head field.
    pf_message_tail_t tail; ///< s5message tail field.
 ...
 */
pf_message_t* s5msg_create(int32_t data_size);
