 ...
 */
typedef struct pf_reap_thread_entry 
{
    pf_dlist_entry_t    thread_list_entry;	///< The real entry in a dlist
    pthread_t           thread_id;			///< The thread id of each reap entry
} pf_reap_thread_entry_t;
...
public:
	 ...
	 */	
	int send_msg_internal(const pf_message_t *msg);
     ...
     */ 	
	pf_message_t*   recv_msg_internal();
	 ...
	 */ 
	pf_message_t* send_msg_wait_reply(const pf_message_t *msg);	
...
public:
	 ...
	 * @retval  -1	Failed.
	 */
	int connect(const char* foreignAddress, unsigned short foreignPort, pf_rcv_type_t autorcv = RCV_TYPE_MANUAL, pf_connect_type_t type = CONNECT_TYPE_TEMPORARY);

	pthread_t	  thread_recv_clt;	  ///< receive thread which to manage receiving pf_message_t s5tcpsocket.
	pthread_t	  thread_recv;		  ///< receive thread which to manage receiving pf_message_t s5tcpsocket of accept.
	pthread_t	  thread_keepalive;   ///< thread which handling keep-alive message.

	pf_rcv_type_t autorcv;			  ///< autoreceive flag, 1:auto 0:manual.
	pf_connect_type_t conntype;		  ///< CONNECT_TYPE_TEMPORARY or CONNECT_TYPE_STABLE
	...
	static  pf_dlist_head_t  reap_thread_list_head; ///< The reaper thread for disconnected socket thread
