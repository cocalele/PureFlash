 ...
 */
void reply_message_read_ok_in_4k(pf_message_t* msg, const char* data, uint32_t len, PS5CLTSOCKET socket)
{
	pf_atomic_add(&reply_ok_count, 1);
	pf_message_t *msg_reply = s5msg_create(0);
	S5ASSERT(msg->head.msg_type == MSG_TYPE_READ ||
		  msg->head.msg_type == MSG_TYPE_S5_STAT ||
	 	  msg->head.msg_type == MSG_TYPE_NIC_CLIENT_INFO);
	memcpy(&msg_reply->head, &msg->head, sizeof(pf_message_head_t));
...
void reply_message_status(pf_message_t* msg, uint16_t status, PS5CLTSOCKET socket)
{
	pf_atomic_add(&reply_ok_count,1);
...
int cachemgr_write_request(pf_message_t *msg, PS5CLTSOCKET socket)
{
	...
	pf_atomic_add(&write_op_count,1);
...
{
	int rc = 0;
	pf_atomic_add(&delete_op_count,1);
