...
#define   SEM_NAME_MOE         "s5lock-moe"	///< request lock's name of semaphore.
#define   SEM_NAME_MOE_ACK     "s5lock-moe-ack"	///< reply lock's name of semaphore.

#define	  MOE_DEFAULT_FILE 		"/usr/tmp/.s5_shr_moe"	///< ipc's file name.
...
{
	int count;
	s5_message_head_t	msg_head[MOE_NOTIFY_ITEMS_MAX];
} moe_notify_shm_t;
