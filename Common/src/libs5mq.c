...
static s5_dlist_head_t sendlist;
static s5_dlist_head_t recvlist;

extern s5_dlist_head_t cndctlist;
extern s5_dlist_head_t workerlist;
...
static int clnt_create_mq_msg(const s5_message_t *sendmsg,
...
static void set_worker_notify_info(s5_worker_status_t status);
static void set_cndct_notify_info(s5_conductor_role_t role);
 ...
 *
 * @param sendmsg[in]   s5_message_t, message client will send out.
 * @param target_id[in] char*,  target identity which is the identity of message receiver, maybe player01...etc.
 * @param recvmsg[out]   s5_message_t, recevie message
 *
 ...
 */
int mq_clnt_msg_send(const s5_message_t *sendmsg,
                    const char *target_id,
    ...
    for (i = 0; i < loop_cnt; i++)
    {
        ...
        S5LIST_FOR_EACH_SAFE(pos, n, count,  &recvlist)
        {
            ...
            if (tmp_mqmsg->msghead.msgid == mqmsg->msghead.msgid  && TRUE == tmp_mqmsg->msghead.recv_flag)
            {
                ...
                memcpy(&recvmsg->head, &tmp_mqmsg->usrdata.s5msg->head, sizeof(s5_message_head_t));
                memcpy(&recvmsg->tail, &tmp_mqmsg->usrdata.s5msg->tail, sizeof(s5_message_tail_t));
 ...
 *
 * @param sendmsg s5_message_t, message client will send
 * @param recvmsg s5_message_t, message client received
 *
...
/**
 * create mqmsg_t message from s5_message_t message
 *
 * @param clnt_send_msg s5_message_t, message user will send
 * @param mqmsg mqmsg_t, message will send by ZMQ
 *
 * @retval 0 for success
 */
int clnt_create_mq_msg(const s5_message_t *sendmsg,
                        const char *target_id,
                        int reply_flag,
                        mqmsg_t *mqmsg)
{
    ...
    memset(mqmsg->usrdata.s5msg, 0, sizeof(s5_message_t));

    memcpy(&mqmsg->usrdata.s5msg->head, &sendmsg->head, sizeof(s5_message_head_t));
    memcpy(&mqmsg->usrdata.s5msg->tail, &sendmsg->tail, sizeof(s5_message_tail_t));
...
int add_msg_to_recvlist(mqmsg_t *mqmsg)
{
    ...
    S5LIST_FOR_EACH_SAFE(pos, n, count, &recvlist)
	{
		...
		if (tmp_msg->msghead.msgid == mqmsg->msghead.msgid)
        {
            ...
            tmp_msg->usrdata.s5msg = (s5_message_t *)malloc(sizeof(s5_message_t));
            assert(tmp_msg->usrdata.s5msg);
            memset(tmp_msg->usrdata.s5msg, 0, sizeof(s5_message_t));

            memcpy(&tmp_msg->usrdata.s5msg->head, &mqmsg->usrdata.s5msg->head, sizeof(s5_message_head_t));
            memcpy(&tmp_msg->usrdata.s5msg->tail, &mqmsg->usrdata.s5msg->tail, sizeof(s5_message_tail_t));
...
void set_worker_notify_info(s5_worker_status_t status)
{
    g_mq_clnt_ctx.worker_status = status;
}

void set_cndct_notify_info( s5_conductor_role_t role)
{
...
int mq_clnt_get_worker_list(s5_dlist_head_t *all_worker_list)
{
	destroy_workerlist_ulc(all_worker_list);

    s5_dlist_entry_t    *tmp_pos    = NULL;
    worker_self_t       *tmp_worker = NULL;
    s5_dlist_entry_t    *outside_n  = NULL;
    int                 count       = 0;
