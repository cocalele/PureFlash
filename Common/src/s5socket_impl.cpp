...
int s5recv_keepalive_reply(void *sockParam, s5_message_t *msg, void *rcvparam);
int s5recv_keepalive(void *sockParam, s5_message_t *msg, void *rcvparam);
...
int CommunicatingSocket::send_msg(s5_message_t *msg)
{
	int rc = 0;
	if(!msg)
	{
...
s5_message_t *CommunicatingSocket::send_msg_wait_reply(const s5_message_t *msg)
{
...
int CommunicatingSocket::send_msg_internal(const s5_message_t *msg)
{
	int rc = -1;
	if(!msg)
	{
...
s5_message_t  *CommunicatingSocket::recv_msg_internal()
{
	s5_message_t *reply_msg = NULL;
	...
	reply_msg = (s5_message_t *)malloc(sizeof(s5_message_t));
	if(!reply_msg)
	{
	...
	headlen = sizeof(s5_message_head_t);
	...
	taillen = sizeof(s5_message_tail_t);
...
void *S5RcvThreadMain(void *paramSock)
{
	...
	s5_reap_thread_entry_t* thread_entry = NULL;
	
	thread_entry = (s5_reap_thread_entry_t*)malloc(sizeof(s5_reap_thread_entry_t));
...
s5_dlist_head_t  S5TCPCltSocket::reap_thread_list_head;
...
S5TCPCltSocket::~S5TCPCltSocket()
{
	...
	if(thread_keepalive)
	{
		s5_wait_thread_end(thread_keepalive);
	}
}
...
void *reaper_handle_thread(void *param)
{
	...
	while(true)
	{
		s5_reap_thread_entry_t* socketThread = NULL;
		...
		socketThread = S5LIST_ENTRY(entry, s5_reap_thread_entry_t, thread_list_entry);
	    s5list_unlock(&S5TCPCltSocket::reap_thread_list_head);	
	
		s5_wait_thread_end(socketThread->thread_id);
		free(entry);	
	}
...
int S5TCPCltSocket::connect(const char *foreignAddress,
						 unsigned short foreignPort, s5_rcv_type_t autorcv, s5_connect_type_t type)
{
	int rc = 0;
	int numsec;
...
S5TCPCltSocket *S5TCPServerSocket::accept(s5_rcv_type_t autorcv)
{
	int newConnSD;
...
int s5recv_keepalive_reply(void *sockParam, s5_message_t *msg, void *param)
{
	int rc = 0;
...
int s5recv_keepalive(void *sockParam, s5_message_t *msg, void *param)
{
	int rc = 0;
	S5TCPCltSocket *socket = (S5TCPCltSocket *)sockParam;
	if(msg)
	{
		...
		s5_message_t *msg_keepalive_reply = s5msg_create(0);
