 ...
 */
typedef int64 (*property_getter)(void* obj);
 ...
 *        int64 depth = queue_get_depth(queue);
 * to register such a property, call spy_register_property like
 *        int64 pf_get_qdepth(s5session* s5s);
 *        spy_register_property("pf_qdepth", pf_get_qdepth, s5session, vt_prop_int64);
 * so far, only property type int64 is supported. i.e. prop_type must be vt_prop_int64
 * used in spy client.
 * @param[in] name 		variable's name, spy client use this name to read this variable.
 ...
 *        int64 depth = queue_set_depth(queue);
 * to register such a property, call spy_register_property like
 *        int64 pf_set_qdepth(s5session* s5s);
 *        spy_register_property("pf_qdepth", pf_set_qdepth, s5session, vt_prop_int64);
 * so far, only property type int64 is supported. i.e. prop_type must be vt_prop_int64
 * used in spy client.
 * @param[in] name 		variable's name, spy client use this name to read this variable.
