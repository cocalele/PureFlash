 ...
 *
 * param[in] outmsg  pf_message_t *, received message.
 ...
 *
 */
typedef void (*callbackfunc)(pf_message_t *outmsg, void *private_arg, const char *sender);
 ...
 */
typedef enum _pf_worker_status_t
{
    WORKER_ST_FREE          = 0,   
    WORKER_ST_NORMAL        = 1,   
    WORKER_ST_BUSY          = 2,   
...
typedef struct __cndct_self_t
{
    ...
    pf_cndct_status_t status;           ///< conductor status
    pf_conductor_role_t role;           ///< condcutor role
...
typedef struct _worker_self_t
{
    ...
    pf_worker_status_t status;          ///< worker status
    uint64  lasttime;                   ///< last heartbeat timestamp
}worker_self_t;
...
typedef struct notify_to_mqcluster
{
	...
	union
	{
		pf_conductor_role_t     cndct_role;     ///< the role of the conductor.
 ...
 *
 * @param[in]	sendmsg   pf_message_t*, message will send out.
 * @param[in]	target_id char*, message receiver, maybe player01, worker01, s5stor01...etc.
 * @param[out]	recvmsg   pf_message_t*, recevie message, caller should free this message.
 ...
 */
int mq_clnt_msg_send(const pf_message_t *sendmsg, const char *target_id, pf_message_t *recvmsg,
	int time_out_sec);
 ...
 *
 * NOTE: 
 *      This function just for conductor,  use pf_dlist_init to initialize a 
 ...
 */
int mq_clnt_get_worker_list(pf_dlist_head_t*workerlist);
