...
struct __log4c_appender
{
  ...
  volatile int					app_isopen;
  volatile void*				app_udata;
  log4c_priority_level_t app_priority;
...
extern log4c_appender_t* log4c_appender_new(const char* a_name)
{
  ...
  this->app_isopen = 0;
  this->app_udata  = NULL;
  this->app_priority = LOG4C_PRIORITY_UNKNOWN;
  pthread_mutex_init(&this->app_mutex, NULL);
  return this;
}
	...
	log4c_logging_event_t	*a_event)
{
	...
	if (!this->app_type->append)
		return 0;
	if(a_event->evt_priority > this->app_priority)
		return 0;

	int rc = 0;
	pthread_mutex_lock(&this->app_mutex);
	if (!this->app_isopen)
...
FINAL:
	pthread_mutex_unlock(&this->app_mutex);
	return rc;
}
...
extern int log4c_appender_set_priority(const char* a_name, log4c_priority_level_t pri)
{
	log4c_appender_t * app = log4c_appender_get(a_name);
	if(!app)
		return -1;
	app->app_priority = pri;
}
