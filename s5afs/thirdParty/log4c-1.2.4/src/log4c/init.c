...
#endif
	if(readlink("/proc/self/exe", rcfiles[3].name, sizeof(rcfiles[3].name)) > 0)
	{
		int pos = rindex(rcfiles[3].name, '/')  - &rcfiles[3].name[0];
		snprintf(&rcfiles[3].name[pos+1], sizeof(rcfiles[3].name) - pos -1, "log4crc");
	}
