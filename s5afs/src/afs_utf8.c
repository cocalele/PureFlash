...
static int encode_utf8(unsigned long u, unsigned char *buf)
{
	...
	if (i == 0) {
		...
		signed int j;
		for (j = i; j > 0; --j) {
			buf[j] = (unsigned char)(0x80 | (u & 0x3f));
			u >>= 6;
		}

		unsigned char mask = (unsigned char)(~(0xFF >> (i + 1)));
		buf[0] = (unsigned char)(mask | u);
	}
	...
	while (1) {
		...
		if (i >= len || c < 0x80 || (c & 0xC0) != 0x80) {
			...
			if (enc_len > 0) {
				...
				if (memcmp(u, re_encoded, (size_t)enc_len) != 0) {
...
{
	return check_utf8(buf, (int)strlen(buf));
}
...
{
	return check_for_control_characters(buf, (int)strlen(buf));
}
